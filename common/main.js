...
$('#choose').on('change', function() {
    ...
    if (files.length > config.maxUploadSize) {
        alert("一次最多同时只可上传4张图片");
        return;
    }
    files.forEach(function(file, i) {
        ...
        reader.onload = function() {
            ...
            // 如果图片大小小于最大上传值，则直接上传
            if (result.length <= config.maxSize) {
                img = null;
...
function compress(img) {
    ...
    // 按照配置的压缩比例进行压缩
    var ndata = canvas.toDataURL('image/jpeg', config.compressRatio);
